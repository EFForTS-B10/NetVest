...
##Results from v1.(C0: default)

#Read in csv and create tibble
v1_C0<-read_csv("data/js_2016_sc11.csv", skip = 6) %>%
rename(step="[step]", meanconsumption = "mean [h_consumption] of turtles")
...
v1_C0_lut_fractions <- v1_C0 %>% 
  ...
  theme_light(base_size = 7) + theme(legend.position = "none")
...
#Read in csv and create tibble
v3_C0<-read_csv("data/C0_with_hhage_simple.csv") %>%
  rename(step="[step]")
...
# NumberHouseholds and household area
coeff <- 1
v3_C0_hh <- v3_C0 %>% 
  select(step, hh.count, hh.area.mean) %>%
  group_by(step) %>%
  summarise_all(list(mean, sd)) %>%
  ggplot(aes(x = step)) +
  geom_ribbon(aes(ymin = hh.count_fn1 - hh.count_fn2, ymax = hh.count_fn1 + hh.count_fn2), fill = "black", alpha = 0.5) +
  geom_line(aes(y = hh.count_fn1 / coeff, colour = "black")) +
  geom_ribbon(aes(ymin = hh.area.mean_fn1 - hh.area.mean_fn2, ymax = hh.area.mean_fn1 + hh.area.mean_fn2), fill = "blue", alpha = 0.5) +
  geom_line(aes(y = hh.area.mean_fn1, colour = "blue")) +
  scale_color_identity() +
  scale_x_continuous(limits = c(1, 50)) +
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~.*coeff, name="Household area")
  ) + 
  labs(
    x = "Simulation time") + # x axis title
    #y = "Carbon (Tons/ha)") +   # y axis title
  #title = "Temporal dynamics under constant price scenarios") +     # main title of figure
  theme_light(base_size = 8) + theme(legend.position = "none") +
theme(
...
##Arrange plots of v1 and v3 for CO
ggarrange(v1_C0_lut_fractions, v3_C0_lut_fractions, v1_C0_consumption, v3_C0_consumption, v1_C0_carbon, v3_C0_carbon, NA, v3_C0_hh, ncol = 2, nrow = 4)
 

plot_grid(v1_C0_lut_fractions, v3_C0_lut_fractions, v1_C0_consumption, v3_C0_consumption, v1_C0_carbon, v3_C0_carbon, NA, v3_C0_hh, labels=c("A", "B"), ncol = 2, nrow = 4, align = "v")
