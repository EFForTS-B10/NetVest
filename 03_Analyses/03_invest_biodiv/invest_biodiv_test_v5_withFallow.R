...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
variables = list(
  "LUT-0-folder" = list(values=rep(c("\"oilpalm_labor_min\"", "\"oilpalm_labor_med\"", "\"oilpalm_labor_high\""), 4)),
  "LUT-1-folder" = list(values=rep(c("\"rubber_labor_min\"", "\"rubber_labor_med\"", "\"rubber_labor_high\""), 4)),
  "price_scenario" = list(values=rep(c(rep("\"constant_prices\"", 3), rep("\"historical_trends\"", 3)), 2)),
  "allow-fallow?" = list(values=c(rep("true", 6), rep("false", 6)))
)
...
library(future)
plan(multisession)
results <- run_nl_all(nl, split=12)

## Attach output:
setsim(nl, "simoutput") <- results
saveRDS(nl, file = file.path(outpath, "sims_wages_fallow_v2.rds"))
...
## Load data:
nl <- readRDS(file.path(outpath, "sims_wages_fallow_v2.rds"))
...
results <- nl@simdesign@simoutput %>% 
  ...
  dplyr::mutate(fallow = dplyr::case_when(fallow == TRUE ~ "fallow",
                                          fallow == FALSE ~ "no fallow")) %>% 
...
#####################
## CHOOSE WHICH PRICE SCENARIO:
choose.price.scenario <- "constant"
choose.price.scenario <- "dynamic"

if(choose.price.scenario == "constant")
{
  
  ## Extract data:
  results.lag <- results %>%
    dplyr::filter(price.scenario == "constant")
...
ggsave(paste0("tradeoff_combined_new", choose.price.scenario, ".png"), path = "03_Analyses/03_invest_biodiv/", width=10, height=10)
...
{
  data %>% 
    ggplot(aes(x=step, y=var.mu, color=labor.op, lty=fallow)) +
...
plot.tinput.mean <- results.lag %>% 
  group_by(labor.op, labor.rm, price.scenario, fallow, step) %>% 
...
ggsave(paste0("time_plots", choose.price.scenario, ".png"), path = "03_Analyses/03_invest_biodiv/", width=16, height=6)
