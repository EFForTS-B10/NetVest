...
results.lag <- results %>% 
  ...
  dplyr::rename(step = `[step]`,
                cons.mean = hh.consumption.mean,  ## prev.cons
                cons.sum = hh.consumption.sum,
                carb.op = lut0.carbon,
                carb.rm = lut1.carbon,
                sar = p.sar,
                tinput.mean = p.tinput.mean, ## prev tinput
                tinput.sum = p.tinput.sum,
                hharea.mean = hh.area.mean,  ## prev hharea
                hharea.sum = hh.area.sum,
                yield.op.sum = lut0.yield.sum,  ## prev yield.op
                yield.op.mean = lut0.yield.mean,
                yield.rm.sum = lut1.yield.sum,  ## prev yield.rm
                yield.rm.mean = lut1.yield.mean,
...
## Technical inputs:
plot.tinput.sum <- results.lag %>% 
  ggplot(aes(x=step, y=p.tinput.sum, color=labor.op, lty=`random-seed`)) +
...
plot.tinput.mean <- results.lag %>% 
  ggplot(aes(x=step, y=p.tinput.mean, color=labor.op, lty = `random-seed`)) +
  facet_wrap(~price.scenario) +
  geom_line(size=1) +
  scale_color_manual(values = col.pal) +
  theme_light(base_size = 11)
...
## Hh area:
plot.hharea.sum <- results.lag %>% 
  ggplot(aes(x=step, y=p.tinput.sum, color=labor.op)) +
  facet_wrap(~price.scenario) +
  geom_line(size=1) +
  scale_color_manual(values = col.pal) +
  theme_light(base_size = 11)
...
results.lag %>% 
  mutate(yield.total = yield.op + yield.rm) %>% 
  group_by(labor.op, labor.rm, price.scenario, step) %>% 
    summarize(cons.mu = mean(cons), cons.sd = sd(cons),
              carb.total = mean(carb.total), carb.sd = sd(carb.total),
              sar = mean(sar), sar.sd = sd(sar),
              tinput = mean(tinput),
              hharea = mean(hharea),
              yield.op = mean(yield.op),
              yield.rm = mean(yield.rm),
              yield.total = mean(yield.total)) %>% 
  ungroup() %>% 
  gather(Variable, value, tinput, hharea, yield.op, yield.rm, yield.total) %>% 
  ggplot(aes(x=step, y=value, color=labor.op)) +
  facet_grid(Variable~price.scenario, scales="free") +
  geom_line(size=1) +
  scale_color_manual(values = col.pal) +
  theme_light(base_size = 11)
