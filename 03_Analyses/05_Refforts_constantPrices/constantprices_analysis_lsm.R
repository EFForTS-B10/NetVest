...
nl <- readRDS(file = file.path("03_Analyses/05_Refforts_constantPrices/constantprices_ff_ineff_outlier.rds"))
results <- nl@simdesign@simoutput

### Without carbon:

classes <- 12
var.names <- c("rubber_area", "oilpalm_area", "household_nr", "household_size", "capitalstock", "consumption_k", "carbon_k", "biodiversity", "area_mn", "ed")
...
ggsave("03_Analyses/05_Refforts_constantPrices/prices_ff_tradeoff_p1.png", units = "cm", width=28, height=18, dpi=300)
...
results_grouped <- results %>% 
  ...
  dplyr::mutate(consumption_carbon = ((consumption_k + carbon_k) / 2)) %>%  
  dplyr::mutate(consumption_biodiversity = ((consumption_k + biodiversity) / 2)) %>%
  dplyr::mutate(consumption_carbon_biodiversity = ((consumption_k + biodiversity + carbon_k) / 3)) %>%
